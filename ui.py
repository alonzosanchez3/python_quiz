...
class QuizInterface:
  ...
  def __init__(self, quiz_brain:QuizBrain):
    ...
    self.true_button = tkinter.Button(image=true_image, highlightthickness=0, command=self.answer_true)
    self.true_button.grid(row=2, column=0)

    false_image = tkinter.PhotoImage(file='./images/false.png')
    self.false_button = tkinter.Button(image=false_image, highlightthickness=0, command=self.answer_false)
